...
import type { Host, Snapshot } from '../../shared/magi/types'
...
os.execv(sys.executable,[sys.executable,'-u',str(d/'magi.py'),'--root',str(r),'--rpc'])`
...
class HostClient {
  ...
  constructor(root: string, resources: string, host?: Host) {
    this.child = spawnProcess(
      host
        ...
        : {
            program: 'python3',
            args: ['-u', join(resources, 'magi.py'), '--root', root, '--rpc'],
            stdio: 'pipe'
          }
    )
      ...
      this.child.stdin.write(
        `${JSON.stringify({
          ...
          files: Object.fromEntries(
            ['magi.py', 'sess', 'SESS-LICENSE'].map((name) => [
              name,
  ...
  close(): void {
    this.fail(new Error('Magi closed'))
    this.child.kill()
  }
}
export class Hosts {
