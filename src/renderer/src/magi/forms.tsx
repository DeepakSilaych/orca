...
import type { Snapshot, Workspace } from '../../../shared/magi/types'
...
export function WorkspaceForm({
  ...
  const [root, setRoot] = useState('~/magi')
  ...
  const submit = async () => {
    ...
    try {
      if (kind === 'host') {
        await window.magi.request('local', 'host_add', { name, ssh, root })
      }
      if (kind === 'repo') {
        await window.magi.request(host, 'repo_register', { name, path, url, utility })
      }
      if (kind === 'ticket') {
        await window.magi.request(host, 'ticket_attach', { workspace: workspace?.id, ticket: name })
      }
      ...
      if (kind === 'workspace') {
        ...
        for (const repo of repos) {
          await window.magi.request(host, 'repo_attach', { workspace: workspace?.id, ...repo })
        }
        done()
      } else {
        done()
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto scrollbar-sleek">
        ...
        >
          ...
          {kind === 'host' && (
            <>
              <Label htmlFor="magi-ssh">SSH alias</Label>
              <Input id="magi-ssh" value={ssh} onChange={(e) => setSsh(e.target.value)} required />
              <Label htmlFor="magi-root">Host folder</Label>
              <Input
                id="magi-root"
                value={root}
          ...
          {kind === 'repo' && (
            <>
              <Label htmlFor="magi-path">Existing path on {host}</Label>
              <div className="flex gap-2">
                ...
                {host === 'local' && (
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      void window.magi.chooseDirectory().then((value) => {
                        if (value) {
                          setPath(value)
                        }
                      })
              ...
              <Label htmlFor="magi-url">Or clone from GitHub</Label>
              <Input
                id="magi-url"
                value={url}
