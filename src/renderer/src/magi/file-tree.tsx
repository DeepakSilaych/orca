...
import type { FileEntry, Workspace } from '../../../shared/magi/types'
import type { OpenFile } from './editor'
export function useFileTree(host: string, workspace: Workspace, active?: OpenFile) {
  const key = `magi.tree:${host}:${workspace.id}`
          ...
          <div key={entry.path}>
            <ActionMenu actions={actions}>
              <button
                ...
                className={`magi-row text-xs ${selected ? 'bg-accent text-foreground' : ''}`}
