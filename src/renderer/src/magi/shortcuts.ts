...
import type { Session, Workspace } from '../../../shared/magi/types'
...
export function useWorkspaceShortcuts(options: {
  ...
  useEffect(
    () =>
      window.magi.onShortcut((shortcut) => {
        if (options.disabled || pending.current || document.querySelector('[data-magi-renaming]')) {
          return
        }
