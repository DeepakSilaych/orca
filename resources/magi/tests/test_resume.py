...
class ImportedResumeTests(unittest.TestCase):
    ...
    def test_resume_command_is_argv_quoted_and_only_started_once(self):
        ...
        with mock.patch.object(test_backend.magi.shutil, 'which', return_value='/bin/tmux'), mock.patch.object(test_backend.magi.subprocess, 'run', return_value=missing), mock.patch.object(test_backend.magi, 'run') as run:
