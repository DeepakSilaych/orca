...
;(async () => {
  ...
  try {
    ...
    const owner = () =>
      execFileSync('tmux', ['list-panes', '-t', `=magi-${id}`, '-F', '#{pane_pid}']).toString()
    const pid = owner()
    ...
    const python = `import os,tty,termios,select; fd=0; old=termios.tcgetattr(fd); tty.setraw(fd); print('MAGI_INPUT_READY',flush=True); data=os.read(fd,1); exec("while select.select([fd],[],[],0.2)[0]:\\n data+=os.read(fd,1024)"); termios.tcsetattr(fd,termios.TCSADRAIN,old); open(${JSON.stringify(capture)},'w').write(data.hex())`
    const command = `python3 -c ` + `'${python.replaceAll("'", "'\\''")}'\r`
    await p.evaluate(() => {
      window.inputOutput = ''
      window.magi.onTerminal((event) => {
        window.inputOutput += event.data || ''
      })
    })
    await p.evaluate(({ key, command }) => window.magi.write(key, command), {
    ...
    if (process.env.MAGI_QA_OUTPUT) {
      await p.screenshot({
        path: path.join(process.env.MAGI_QA_OUTPUT, 'magi-resizable-panels.png')
      })
    }
    console.log(
