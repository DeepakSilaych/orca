...
class AgentIdentityTests(unittest.TestCase):
    def test_executables_and_runtime_entrypoints(self):
        ...
        for command, expected in cases.items():
            with self.subTest(command=command): self.assertEqual(test_backend.magi.agent_command(command), expected)
    ...
    def test_foreground_identity_ignores_background_and_other_sessions(self):
        ...
        result = test_backend.magi.foreground_agents(panes, table, [{'id':'a'}, {'id':'b'}])
        self.assertEqual(result, {'a':'gemini', 'b':None})
        self.assertEqual(test_backend.magi.foreground_agents(panes, '100 1 100 100 /bin/zsh', [{'id':'a'}]), {'a':None})

    def test_unavailable_probe_does_not_claim_exit(self):
        with mock.patch.object(test_backend.magi, 'run', side_effect=ValueError('unreachable')):
            self.assertIsNone(test_backend.magi.Backend.terminal_agents(None, [{'id':'a','started':True}]))
