...
class LayoutTests(unittest.TestCase):
    ...
    def test_nested_split_pruning_and_persistence(self):
        ...
        self.assertEqual(test_backend.magi.Backend(self.backend.root).ws(ws['id'])['layouts'][first['id']]['ratio'], 0.7)
        with mock.patch.object(test_backend.magi, 'run') as run:
            self.backend.terminal_remove(ws['id'], second['id'])
            self.assertEqual(run.call_args.args[0], ['tmux', 'kill-session', '-t', '=magi-' + second['id']])
