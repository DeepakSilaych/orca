...
const fs = require('node:fs'),
  ...
  os = require('node:os')
const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'magi-shortcut-qa-'))
;(async () => {
  ...
  try {
    ...
    if (process.env.MAGI_QA_OUTPUT) {
      await p.screenshot({ path: path.join(process.env.MAGI_QA_OUTPUT, 'rename-workspace.png') })
    }
    ...
    if (process.env.MAGI_QA_OUTPUT) {
      await p.screenshot({ path: path.join(process.env.MAGI_QA_OUTPUT, 'rename-terminal.png') })
    }
    await key('w')
