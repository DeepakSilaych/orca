...
BACKEND = Path(__file__).resolve().parents[1] / 'backend' / 'magi.py'
spec = importlib.util.spec_from_file_location('magi', BACKEND)
magi = importlib.util.module_from_spec(spec)
spec.loader.exec_module(magi)


class WorkspaceTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='magi-test-')
        self.addCleanup(self.temp.cleanup)
        self.base = Path(self.temp.name)
        self.backend = magi.Backend(self.base / "Magi root's space")
        self.repos = {}
        for name in ('web', 'api'):
            p = self.base / name
            p.mkdir()
            self.git(p, 'init', '-b', 'main')
            self.git(p, 'config', 'user.name', 'Magi Test')
            self.git(p, 'config', 'user.email', 'magi-test@example.invalid')
    ...
    def test_permanent_general_per_host_and_restart(self):
        ...
        restarted = magi.Backend(self.backend.root)
        ...
        another = magi.Backend(self.base / 'another-host')
    ...
    def test_large_diff_is_bounded(self):
        ...
        self.assertLessEqual(len(result['text'].encode()),magi.MAX_TEXT)
    ...
    def test_appearance_preferences_persist_and_validate(self):
        ...
        other=magi.Backend(self.backend.root)
    ...
    def test_clone_uses_gh_and_infers_name(self):
        ...
        with mock.patch.object(magi.shutil,'which',return_value='/usr/bin/gh'),mock.patch.object(magi,'run',side_effect=fake_run):
    ...
    def test_gh_missing_is_actionable_and_keeps_registry(self):
        with mock.patch.object(magi.shutil,'which',return_value=None):
    ...
    def test_terminal_reuses_live_tmux_and_exports_cli(self):
        ws=self.create([]);t=ws['terminals'][0];session='magi-'+t['id']
        ...
        target=magi.text(magi.run(['tmux','list-panes','-s','-t',session,'-F','#{pane_id}'])).splitlines()[0]
        first=magi.run(['tmux','display-message','-p','-t',target,'#{pane_pid}'])
        self.backend.terminal_prepare(ws['id'],t['id'])
        self.assertEqual(first,magi.run(['tmux','display-message','-p','-t',target,'#{pane_pid}']))
        self.assertEqual(prepared['env']['MAGI_WORKSPACE'],ws['id'])
        command='echo MAGI_READY; printf "%s\\n" "$MAGI_ROOT"; magi status --json'
        magi.run(['tmux','send-keys','-t',target,command,'Enter'])
        deadline=time.monotonic()+8
        content=''
        while time.monotonic()<deadline:
            content=magi.text(magi.run(['tmux','capture-pane','-p','-t',target,'-S','-100']))
