...
const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'magi-layout-qa-'))
;(async () => {
  ...
  try {
    ...
    await p.waitForFunction(
      async ({ wid, tid }) =>
        (await window.magi.request('local', 'snapshot')).workspaces.find((w) => w.id === wid)
    ...
    await p.waitForFunction(
      async ({ a, b }) => {
        const ws = (await window.magi.request('local', 'snapshot')).workspaces
    ...
    const update = await p.evaluate(() => window.magi.getUpdate())
    if (update.releaseUrl !== 'https://github.com/DeepakSilaych/orca/releases') {
      throw new Error('Incorrect release source')
    }
    if (process.env.MAGI_QA_OUTPUT) {
      await p.screenshot({ path: path.join(process.env.MAGI_QA_OUTPUT, 'magi-settings.png') })
    }
    if (errors.length) {
