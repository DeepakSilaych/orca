...
const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'magi-polish-qa-'))
;(async () => {
  ...
  try {
    ...
    const savedScroll = () =>
      p.evaluate(() => {
        const key = Object.keys(localStorage).find((k) => k.startsWith('magi.editor:'))
    ...
    await p.evaluate(
      (key) => window.magi.write(key, "bash -c 'exec -a codex sleep 60'\r"),
      identityKey
    )
    await expect
    ...
    await p.evaluate((key) => window.magi.write(key, '\x03'), identityKey)
    await expect
      .poll(async () => (await request('status', { workspace: a.id })).agents[identityTerminal.id])
      .toBe(null)
    await p.getByRole('button', { name: 'Refresh repositories', exact: true }).click()
    ...
    if (process.env.MAGI_QA_OUTPUT) {
      await p.screenshot({ path: path.join(process.env.MAGI_QA_OUTPUT, 'v023-ui.png') })
    }
