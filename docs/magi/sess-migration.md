...
Implementation branch: magi/sess-workspaces.

## Product boundary

Use Orca's existing renderer, shadcn/Radix components, terminal panes, settings, and source-control views. Retire the separate Magi renderer. Preserve the upstream MIT license and attribution.
...
The old Magi backend is a reference for worktree layout, gh cloning, host presets, and Genral protections. It is not a second UI or a second authoritative process registry in this fork.
